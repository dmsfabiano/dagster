...
from dagster._daemon.asset_daemon import (
    ...
    _get_pre_evaluation_group_raw_cursor,
...
def test_error_loop_before_cursor_written(daemon_not_paused_instance, crash_location):
    ...
    for trial_num in range(3):
        ...
        with pendulum.test(test_time):
            ...
            cursor = _get_pre_evaluation_group_raw_cursor(instance)
            assert not cursor
...
def test_error_loop_after_cursor_written(daemon_not_paused_instance, crash_location):
    ...
    for trial_num in range(3):
        ...
        with pendulum.test(test_time):
            ...
            # Same cursor due to the retry
            retry_cursor = _get_pre_evaluation_group_raw_cursor(instance)
    ...
    with pendulum.test(test_time):
        ...
        # Cursor has moved on
        moved_on_cursor = _get_pre_evaluation_group_raw_cursor(instance)
